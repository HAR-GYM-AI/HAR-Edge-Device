...
This file is kept for backwards compatibility and includes the full
implementation. When possible, prefer the packaged entrypoint
`src.har_edge_device.cli:main` (created during cleanup).

When executed, the script will attempt to delegate to the packaged
entrypoint. If the package cannot be imported (development mode), the
legacy implementation (this file) will continue to execute normally.
"""

# Attempt to delegate to package entrypoint for a cleaner import layout.
try:
    ...
    from src.har_edge_device import cli as _pkg_cli

    if hasattr(_pkg_cli, 'main'):
        # Run packaged entrypoint and exit
        _pkg_cli.main()
        raise SystemExit(0)
except Exception:
    # Could not delegate - continue with legacy implementation below
    pass
...
import numpy as np
import pandas as pd
from src.post_process_data import extract_temporal_features, extract_enhanced_frequency_features
